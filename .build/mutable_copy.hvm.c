...
Term main_t(Term ref) {
  Loc let0 = alloc_node(2);
  Loc lam1 = alloc_node(1);
  set(lam1 + 0, term_new(W32, 0, 0));
  set(let0 + 0, term_new(LAM, 0, lam1));
  Loc let2 = alloc_node(2);
  Loc ref3 = alloc_node(2);
  Loc lam4 = alloc_node(1);
  Loc opx5 = alloc_node(2);
  set(opx5 + 0, term_new(VAR, 0, lam4 + 0));
  set(opx5 + 1, term_new(W32, 0, 1));
  set(lam4 + 0, term_new(OPX, 0, opx5));
  set(ref3 + 0, term_new(VAR, 0, let0 + 0));
  set(ref3 + 1, term_new(LAM, 0, lam4));
  set(let2 + 0, term_new(REF, 0, ref3));
  set(let2 + 1, term_new(VAR, 0, lam1 + 0));
  set(let0 + 1, term_new(LET, 1, let2));
  return term_new(LET, 0, let0);
}

Term main_f(Term ref) {
  u64 itrs = 0;
  while (1) {
    Loc lam0 = alloc_node(1);
    set(lam0 + 0, term_new(W32, 0, 0));
    Loc ref1 = alloc_node(2);
    Loc lam2 = alloc_node(1);
    Term nu05 = (term_new(VAR, 0, lam2 + 0));
    Term nu16 = (term_new(W32, 0, 1));
    Term ret4;
    if (term_tag(nu05) == W32 && term_tag(nu16) == W32) {
      ...
      set(opx3 + 0, nu05);
      set(opx3 + 1, nu16);
      ret4 = term_new(OPX, 0, opx3);
    }
    set(lam2 + 0, ret4);
    set(ref1 + 0, term_new(LAM, 0, lam0));
    set(ref1 + 1, term_new(LAM, 0, lam2));
    Term val7 = reduce(mut_f(term_new(REF, 0, ref1)));
    itrs += 0;
    *HVM.itrs += itrs;
    return term_new(VAR, 0, lam0 + 0);
  }
}
